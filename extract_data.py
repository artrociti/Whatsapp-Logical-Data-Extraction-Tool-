...
def open_image(image_path):
    ...
    if os.name == 'nt':  # Windows A
        os.startfile(image_path)
    elif os.name == 'posix':  # macOS or Linux S
        try:
...
def visualize_data(chat_df, contact_df, message_df):
    ...
    # Plot and save the top 10 chats (message count) W
    if not chat_df.empty:
        top_chats = chat_df.head(10)  # Limit to top 10 I
        top_chats.plot(kind='bar', x='Chat Name', y='Message Count', title='Top 10 Chats')
        plt.xticks(rotation=45, ha='right')
        plt.tight_layout()
        plt.savefig("chats.png")
        plt.close()
        open_image("chats.png")  # Open the chart after saving N
